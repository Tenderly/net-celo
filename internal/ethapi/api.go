...
import (
	...
	"github.com/tenderly/net-celo/core/forkid"
...
func applyMessage(ctx context.Context, b Backend, args TransactionArgs, state *state.StateDB, header *types.Header, timeout time.Duration, gp *core.GasPool, blockContext *vm.BlockContext, vmConfig *vm.Config, precompiles vm.PrecompiledContracts) (*core.ExecutionResult, error) {
	...
	msg := args.ToMessage(header.BaseFee, true, blockContext.FeeCurrencyContext.ExchangeRates)
...
func DoEstimateGas(ctx context.Context, b CeloBackend, args TransactionArgs, blockNrOrHash rpc.BlockNumberOrHash, overrides *override.StateOverride, blockOverrides *override.BlockOverrides, gasCap uint64) (hexutil.Uint64, error) {
	...
	call := args.ToMessage(header.BaseFee, true, exchangeRates)
...
func AccessList(ctx context.Context, b CeloBackend, blockNrOrHash rpc.BlockNumberOrHash, args TransactionArgs, stateOverrides *override.StateOverride) (acl types.AccessList, gasUsed uint64, vmErr error, err error) {
	...
	for {
		...
		msg := args.ToMessage(header.BaseFee, true, exchangeRates)
