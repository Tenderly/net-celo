...
func TestProcessVerkle(t *testing.T) {
	var (
		...
		intrinsicContractCreationGas, _ = IntrinsicGas(code, nil, true, true, true, true, nil)
		...
		intrinsicCodeWithExtCodeCopyGas, _ = IntrinsicGas(codeWithExtCodeCopy, nil, true, true, true, true, nil)
...
func TestProcessParentBlockHash(t *testing.T) {
	...
	checkBlockHashes := func(statedb *state.StateDB) {
		...
		for i := 1; i <= num; i++ {
			...
			vmContext := NewEVMBlockContext(header, nil, new(common.Address), testVerkleChainConfig, statedb)
